...
export async function getAstroCache(
  ...
  try {
    ...
    const where: any = { userId, kind };
    if (date) {
      where.date = date;
    }
    const doc = await prisma.astroCache.findFirst({
      where,
      orderBy: { updatedAt: "desc" },
    });
    if (!doc) {
      return res.json({ cached: false, payload: null });
    }
    return res.json({
      cached: true,
      kind: doc.kind,
...
export async function putAstroCache(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  try {
    const userId = (req as any).userId as string;
    const { kind, date, payload } = req.body as {
      kind: string;
      date?: string;
      payload: any;
    };
    if (!kind || payload == null) {
      throw new AppError("kind and payload are required", 400);
    }
    const key = `${userId}:${kind}:${date ?? "na"}`;
